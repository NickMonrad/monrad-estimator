...
`DATABASE_URL` is the source of truth. The command reads it from the process environment first, then `server/.env` (or the path set by `MONRAD_ENV_FILE`), and writes a verified non-empty custom-format dump to `backups/`. Host mode (default — conservative, no automatic Docker detection) uses the host `pg_dump`; authority and query-string database passwords are removed from the `--dbname` argument and passed through the `PGPASSWORD` environment variable instead. For Docker-based PostgreSQL, set `MONRAD_DB_MODE=docker` explicitly or set `MONRAD_DB_CONTAINER` (activates Docker mode implicitly) — the command never probes the Docker daemon. `MONRAD_DB_CONTAINER` overrides the container name (default `monrad-pg`). Set `MONRAD_DB_MODE=host` to force host mode explicitly. Error messages report the executable and exit status without revealing the database URL or password. The command fails rather than silently backing up an unconfigured or different database. Backup regression tests cover mode selection, configuration precedence, write atomicity, verification, cleanup, and all failure paths. They run in Linux and Windows CI as part of `npm test` and `npm run validate`.
