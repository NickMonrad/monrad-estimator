...
export async function runE2eLocal({ spawn: spawnOption, runCommand: runCommandOverride, withIsolatedTestDatabase: withIsolatedTestDatabaseOverride, loadLocalEnvironment: loadEnvOverride, waitForMonradApi: waitForMonradApiOverride, waitForClient: waitForClientOverride, waitForProxy: waitForProxyOverride } = {}) {
  ...
  if (!jwtSecret || jwtSecret === 'change-me-in-production' || jwtSecret.length < 32) {
    ...
    return { exitCode: 1, primaryChildFailure: null, cleanupErrors: [] }
  }
  ...
  try {
    ...
    if (primaryChildFailure) {
      cleanupErrors.push({ type: 'runner', error: primaryChildFailure.message })
    } else {
  ...
  return {
    ...
    primaryChildFailure,
    cleanupErrors: cleanupErrors.filter(e => {
   ...
   * Records the first unexpected exit/error as `primaryChildFailure`
  ...
  function monitorChild(child, label) {
    ...
    const failure = new Promise((_, reject) => {
      child.on('error', err => {
        ...
        if (!primaryChildFailure) {
          primaryChildFailure = new Error(msg)
          internalAbort.abort()
        }
        reject(primaryChildFailure)
      })
      child.on('exit', code => {
        if (expectedShutdown) return
  ...
  async function stopChildren() {
    ...
    if (failures.length > 0) {
      ...
      cleanupErrors.push({ type: 'process termination', error: messages })
    }
  }
  ...
  async function waitFor(label, check) {
    ...
    while (Date.now() < deadline) {
      if (combinedSignal.aborted) {
        if (primaryChildFailure) throw primaryChildFailure
  ...
  function computeExitCode() {
    ...
    if (primaryChildFailure || cleanupErrors.some(e => e.type !== 'child-process termination')) return 1
    return 0
  }
}
