...
`beforeEach` seeds a project with Developer (264 h) and Tech Lead (8 h) tasks via CSV import, navigates to Timeline, and runs Quick schedule. The ramp-up test additionally creates a named resource via `POST /named-resources` (which since #403 sends no capacity fields and generates the new profile from the role's authoritative profile with `source=DERIVED` and `legacy.writer=ROLE_DEFAULT`), then converts that generated clone via raw SQL into a mapper-derived scalar profile (`source=FIXED`, `planningBasis=DEMAND_FOLLOWING`, `defaultPercent=100`, fully populated legacy payload) — the **LEGACY_MAPPER_SCALAR** state the optimiser may ramp up. A profile-less named resource is an integrity violation since the runtime cutover (issue #418) and fails closed. The feature start week is manually overridden to week 5 and re-scheduled to push Developer first demand past week 0. Each test has a 90 s timeout.
