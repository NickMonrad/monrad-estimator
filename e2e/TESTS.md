...
| removes a dependency whose remove control sits under an intervening feature bar | Creates three features and both `A → B` and `A → C` edges, proves the `A → C` midpoint lands inside the intervening feature's bar, selects the connector from a hit-tested visible segment, asserts the remove control (not the bar) owns the covered point, clicks it with the mouse, and asserts the 200 DELETE, the surviving `A → B` edge, and persistence after reload |
