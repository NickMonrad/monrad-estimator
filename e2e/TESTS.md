...
| deterministic ramp-up apply creates optimiser-derived profile, cross-view parity and snapshot undo | Enables ramp-up checkbox, runs optimiser, finds a candidate with positive `suggestedStartWeek` for the Developer NO_PROFILE named resource, applies via UI. Verifies: the apply payload carries the complete scope; SQL confirms exactly one `CapacityProfile` with `ownerKind=NAMED_PERSON`, `planningBasis=AVAILABILITY_WINDOW`, `source=DERIVED`, `startWeek=selectedSuggestion`, `endWeek=null`, `defaultPercent=100`, `legacy=[version=1, writer=RESOURCE_OPTIMISER]`, and zero `CapacitySegment` rows; all seven NamedResource compatibility fields match the profile projection; Resource Profile reader shows `resolutionSource=PROFILE` with matching start/week/percentage/mode; Timeline reader shows matching start/week/percentage/mode. After rollback, asserts via SQL that the profile and segments are removed, NamedResource fields restored, Resource Profile and Timeline values restored — no conditional bypass. |
