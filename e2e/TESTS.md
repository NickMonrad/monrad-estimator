...
| profile-first ramp-up applies authoritative capacity and supports undo | Adds a named person through Resource Profile API, captures pre-apply state, submits a direct `POST /optimise/apply` with positive `suggestedStartWeek`, verifies `capacity-profiles` returns exactly one `NAMED_PERSON` / `AVAILABILITY_WINDOW` / `DERIVED` profile with zero segments and the correct start week; asserts cross-view parity across Resource Profile and Timeline; rolls back via snapshot ID and verifies count, profile state, and compatibility fields are restored |
