...
test.describe('Backlog', () => {
  ...
  test('duplicates every backlog level, preserves metadata, and edits independently', async ({ page }) => {
    ...
    const sourceTaskRow = page.getByText(taskName, { exact: true }).locator('..').locator('..').locator('..')
    await sourceTaskRow.hover()
    await sourceTaskRow.getByTitle('Duplicate task').click()
    await expect(page.getByText(`Copy of ${taskName}`, { exact: true })).toBeVisible({ timeout: 10_000 })
    const duplicateTaskRow = page.getByText(`Copy of ${taskName}`, { exact: true }).locator('..').locator('..').locator('..')
    await duplicateTaskRow.hover()
    await duplicateTaskRow.getByRole('button', { name: 'Edit' }).click()
    ...
    const sourceStoryRow = page.getByText(storyName, { exact: true }).locator('..').locator('..')
    await sourceStoryRow.hover()
    await sourceStoryRow.getByTitle('Duplicate story').click()
    const sourceFeatureRow = page.getByText(featureName, { exact: true }).locator('..').locator('..')
    await sourceFeatureRow.hover()
    await sourceFeatureRow.getByTitle('Duplicate feature').click()
    const sourceEpicRow = page.getByText(epicName, { exact: true }).locator('..').locator('..').locator('..')
    await sourceEpicRow.hover()
    await sourceEpicRow.getByTitle('Duplicate epic').click()
...
test.describe('Grid Entry', () => {
  test('creates, validates, persists and edits a keyboard-first backlog grid', async ({ page }) => {
    const projectName = `E2E Grid Entry ${Date.now()}`
    await login(page)
    ...
    await expect(page.getByRole('button', { name: 'Open Grid Entry' })).toBeVisible()
    await expect(page.getByRole('button', { name: /import csv/i })).toBeVisible()
    await expect(page.getByRole('button', { name: /export csv/i })).toBeVisible()
    await page.getByRole('button', { name: 'Open Grid Entry' }).click()

    const addRow = async (type: string) => {
      await page.getByLabel('New row type').selectOption(type)
      await page.getByRole('button', { name: /add row/i }).click()
    }
    await addRow('epic')
    await page.getByLabel('Row 5 name').fill('Grid Epic')
    await addRow('feature')
    await page.getByLabel('Row 6 epicName').fill('Grid Epic')
    await page.getByLabel('Row 6 name').fill('Grid Feature')
    await addRow('story')
    await page.getByLabel('Row 7 epicName').fill('Grid Epic')
    await page.getByLabel('Row 7 featureName').fill('Grid Feature')
    await page.getByLabel('Row 7 name').fill('Grid Story')
    await addRow('task')
    await page.getByLabel('Row 8 epicName').fill('Grid Epic')
    await page.getByLabel('Row 8 featureName').fill('Grid Feature')
    await page.getByLabel('Row 8 storyName').fill('Grid Story')
    await page.getByLabel('Row 8 name').fill('Grid Task 1')
    await page.getByLabel('Row 8 resource type').selectOption({ label: 'Developer' })
    await page.getByLabel('Row 8 hoursEffort').fill('-1')
    await addRow('task')
    await page.getByLabel('Row 9 epicName').fill('Grid Epic')
    await page.getByLabel('Row 9 featureName').fill('Grid Feature')
    await page.getByLabel('Row 9 storyName').fill('Grid Story')
    await page.getByLabel('Row 9 name').fill('Grid Task 2')
    await page.getByLabel('Row 9 resource type').selectOption({ label: 'Developer' })
    await page.getByLabel('Row 9 hoursEffort').fill('8')
    await page.getByLabel('Row 8 name').evaluate((element) => {
      const clipboardData = new DataTransfer()
      clipboardData.setData('text/plain', 'Grid Task 1\tactive\tDeveloper\t-1\nGrid Task Pasted\tactive\tDeveloper\t8')
      element.dispatchEvent(new ClipboardEvent('paste', { clipboardData, bubbles: true }))
    })
    await page.getByRole('button', { name: /save grid entry/i }).click()
    await expect(page.getByText(/Hours must be a non-negative number/)).toBeVisible()
    await page.getByLabel('Row 8 hoursEffort').fill('4')
    const firstCommit = page.waitForResponse(response => response.url().includes('/backlog/grid-commit') && response.request().method() === 'POST')
    await page.getByRole('button', { name: /save grid entry/i }).click()
    const firstCommitBody = await firstCommit.then(response => response.json()) as { rowIds?: Array<string | null> }
    expect(firstCommitBody.rowIds?.[4]).toEqual(expect.any(String))
    await expect(page.getByText('Saved')).toBeVisible()
    await page.getByLabel('Row 5 name').fill('Grid Epic Same Session')
    const secondCommit = page.waitForRequest(request => request.url().includes('/backlog/grid-commit') && request.method() === 'POST')
    await page.getByRole('button', { name: /save grid entry/i }).click()
    const secondCommitBody = JSON.parse((await secondCommit).postData() ?? '{}') as { rows: Array<{ id?: string; name?: string }> }
    expect(secondCommitBody.rows[4]).toEqual(expect.objectContaining({ id: expect.any(String), name: 'Grid Epic Same Session' }))
    await expect(page.getByText('Saved')).toBeVisible()
    await expect(page.getByLabel('Row 5 name')).toHaveValue('Grid Epic Same Session')
    await page.getByRole('button', { name: /tree view/i }).click()
    await page.reload()
    await expect(page.getByText('Grid Epic Same Session')).toBeVisible({ timeout: 10_000 })

    await page.getByRole('button', { name: 'Open Grid Entry' }).click()
    const taskRow = page.locator('tr').filter({ has: page.getByLabel('Row 8 name') })
    await taskRow.getByLabel('Row 8 name').fill('Grid Task Updated')
    await taskRow.locator('input[aria-label*="hoursEffort"]').fill('16')
    await page.getByRole('button', { name: /save grid entry/i }).click()
    await expect(page.getByText('Saved')).toBeVisible()
    await page.reload()
    await expect(page.getByText('Grid Epic Same Session')).toBeVisible({ timeout: 10_000 })
    await page.getByText('Grid Epic Same Session').first().click()
    await expect(page.getByText('Grid Feature')).toBeVisible()
    await page.getByText('Grid Feature').first().click()
    await page.getByText('Grid Story').first().click()
    await expect(page.getByText('Grid Task Updated')).toBeVisible({ timeout: 10_000 })
    await page.reload()
    await page.getByText('Grid Epic Same Session').first().click()
    await page.getByText('Grid Feature').first().click()
    await page.getByText('Grid Story').first().click()
    await expect(page.getByText('Grid Task Updated')).toBeVisible()
    await page.getByRole('button', { name: 'Open Grid Entry' }).click()

    await page.getByLabel('Row 5 name').fill('Unsaved Grid Epic')
    let warned = false
    page.once('dialog', async dialog => { warned = /unsaved/i.test(dialog.message()); await dialog.dismiss() })
    await page.getByRole('button', { name: /tree view/i }).click()
    expect(warned).toBe(true)
    await expect(page.getByRole('button', { name: /save grid entry/i })).toBeVisible()
