...
test.describe('Resource Allocation', () => {
  ...
  test('Squad Planner apply preserves one planned-resource segmented profile through the safe editor', async ({ page }) => {
    ...
    await expect(page.getByTestId(`named-resource-row-${before.namedResourceId}`).getByRole("link", { name: /Open Squad Planner/i })).toBeVisible({ timeout: 5_000 })
    await expect(page.getByRole("link", { name: "Open Squad Planner" })).toHaveAttribute('href', `/projects/${projectId}/timeline?panel=squad-planner`)
    await expect(page.getByTestId('named-resource-row-' + before.namedResourceId).getByRole('button', { name: /Edit profile|Create profile/i })).toHaveCount(0)
