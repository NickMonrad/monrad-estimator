...
| Capacity Plan | Varies by week (availability pattern) |
| TIMELINE allocation | Fixed for selected weeks (availability pattern) |
| FULL_PROJECT allocation | Fixed for whole project (availability pattern) |
| EFFORT allocation | As needed (availability pattern) |
...
| T&M | As needed (availability) / Bill actual scheduled days (commercial) |
| Pro-rata | Bill planned allocation |

## Availability patterns (current UI labels)

| Stored/internal mode | User-facing availability pattern |
|---|---|
| `EFFORT` | As needed |
...
The stored enum values (`EFFORT`, `FULL_PROJECT`, `TIMELINE`, `CAPACITY_PLAN`) remain
unchanged across API contracts. Only the user-facing label has changed.
...
Billable days: 52.8
| **Availability pattern** | Profile `planningBasis` mapped via `formatPlanningBasis` | Shows plain-English availability pattern |

### PR #356 implementation (merged)

After PR #356 merged, the CSV export in `useResourceProfileExport.ts` implemented the following
| **Available from / Available to** | Profile window or legacy NR fields | Profile-first precedence |
...
the corresponding columns. Available from/to fields also prefer the profile
